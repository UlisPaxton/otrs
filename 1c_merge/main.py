...
WORK_DIRECTORY = '/home/astral/otrs/1c_merge/'
...
DEBUG_level = int( config['GLOBAL']['DEBUG_LEVEL'] )  # 0 - рабочее логирование, 1 - +логирование ошибок предоставленных данных, 2 - +логирование логики
...
def merge(client, first_tn, second_tn):

    log(f"[?]К тикету {first_tn} мёржим содержимое тикета {second_tn}")

    ticket1_id = client.ticket_get_by_number(first_tn)
    ticket = client.ticket_get_by_id(ticket1_id.tid, articles=True)
    ...
    ticket2_id = client.ticket_get_by_number(second_tn)
    ticket2 = client.ticket_get_by_id(ticket2_id.tid, articles=True, attachments=True)
    ...
    for article in ticket2.articles:
        ...
        if len(article.attachments) > 0:
            ...
            client.ticket_update(ticket.tid, article=article, attachments=article.attachments)
        else:
            client.ticket_update(ticket.tid, article=article)

        sleep(float(config['otrs']['operations_interval']))

    check_value(
        client.ticket_update(ticket2.tid, State="removed"),
...
pattern_1c_tn = re.compile(r'(ED-\d{6})')
...
found_tickets = Connection.execute("select tn, title from ticket where (queue_id=8 and (ticket_state_id=1 or ticket_state_id=4)) ", dictionary=True)

tickets_1c = list()       # берём все новые тикетв из очереди с указанным queue_id ??? а если родительский тикет уже не новый?

for row in found_tickets:
    result = pattern_1c_tn.findall(row['title'])  # ищем в темах номер тикета 1с
...
sleep(float(config['otrs']['operations_interval']))  # при несоблюдении интервала обращений к api апач может рвать соединение
